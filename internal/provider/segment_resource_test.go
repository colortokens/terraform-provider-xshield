...
func testAccSegmentConfig(name, description string, timeline int64) string {
	return testAccConfig(fmt.Sprintf(`
resource "xshield_segment" "test" {
  tag_based_policy_name      = %q
  description                = %q
  criteria                   = "tags.Environment = 'terraform-acceptance'"
...
func TestAccSegmentResource(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccSegmentConfig(name, "created by acceptance test", 10),
			...
			{
				Config: testAccSegmentConfig(updatedName, "updated by acceptance test", 20),
				ConfigStateChecks: []statecheck.StateCheck{
...
func TestAccSegmentRejectsInvalidValues(t *testing.T) {
	tests := map[string]struct {
		...
		"timeline below minimum": {
			config:      testAccSegmentConfig(acctest.RandomWithPrefix("tf-acc-segment"), "invalid timeline", 0),
		...
		Steps: []resource.TestStep{
			{
				...
				Config: testAccSegmentConfig(name, "read by acceptance test", 10) + fmt.Sprintf(`
